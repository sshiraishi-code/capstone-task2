...
class TabularProcessor:
    ...
    def save_splits(self, output_path: Path):
        ...
        if self.train_data is not None:
            self.train_data.to_csv(os.path.join(output_path, 'train_data.csv'), index = False)
        if self.test_data is not None:
            self.test_data.to_csv(os.path.join(output_path, 'test_data.csv'), index = False)
        if self.valid_data is not None:
            self.valid_data.to_csv(os.path.join(output_path, 'valid_data.csv'), index = False)

    def impute_categorical_data(self, col_names: list[str]):
        """ Impute NaN with -1 as a new category. """
        for c in col_names:
            if self.data[c].dtypes != 'object':
                v = self.data[c].unique()
                minval = min([x for x in v if ~np.isnan(x)])
                for i in self.data[self.data[c].isnull()].index:
                    self.data.at[i, c] = minval - 1
            else:
                raise ValueError(f'{self.data[c].dtypes} data type not supported yet.')

    def impute_weight(self, col_names: list[str]):
        """ Impute NaN with average value from the same category. """
        for c in col_names:
            if self.data[c].dtypes != 'object':
                for i in self.data[self.data[c].isnull()].index:
                    temp = self.data[(~self.data[c].isnull()) & (self.data["gender"] == self.data.iloc[i]["gender"]) & (self.data["centerid"] == self.data.iloc[i]["centerid"])]
                    self.data.at[i, c] = temp[c].mean()
            else:
                raise ValueError(f'{self.data[c].dtypes} data type not supported yet.')
    ...
    def upsample_train_data(self, total_rows: int = 1200):
        """  Upsakmple train data with 50-50. """
        idx = self.train_data.index
        rint = np.random.randint(0, len(idx), total_rows - len(idx))
        self.train_data = pd.concat([self.train_data.loc[idx[rint]], self.train_data])
    
    def one_hot_encoder(self, col_names: list[str]): 
        for c in col_names:      
            # Get one hot encoding of columns B
            one_hot = pd.get_dummies(self.data[c], dtype='int32', prefix='gender')
